...
from app.schemas import ReviewEnqueueRequest, ReviewQueue, ReviewToday, ReviewUpdateRequest
...
def enqueue(
    expression_id: str,
    payload: ReviewEnqueueRequest | None = None,
    ...
    score = payload.score if payload else 55
    expression = repository.get_expression(expression_id)
    next_review = calculate_sm2(score=score, repetition=0, interval=1, ef=2.5, now=datetime.now(timezone.utc))
...
def update_review(
    ...
    queue.last_score = payload.score
