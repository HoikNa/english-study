...
| D-06 Android dev build v5 (expo-updates) | Pass built | EAS development APK build `eb3132be-c2cf-4256-bd72-1acf2f045e84` finished with `expo-updates@~56.0.17` bundled, `updates.url` configured for project `57cccf56-7494-4844-a508-89b6cb754c3a`, and `runtimeVersion={ policy: "appVersion" }`. eas.json now maps profiles to channels `development` / `preview` / `production`. APK: `https://expo.dev/artifacts/eas/v3t5J36UyQBq3hMYSeiWvA.apk`. |
| D-07 Initial OTA publish | Pass published | `eas update --channel development --environment development --message "OTA setup initial publish (2026-05-27)"` succeeded. Update group `22c8b604-d0eb-4a87-9c11-fea4d44526d1`, Android update id `019e6996-b348-7598-ba92-c1f20170d5c3`. |
| D-08 OTA env var fix | Pass device | OTA bundles inherited no `EXPO_PUBLIC_*` env vars because EAS Update only reads from EAS environment system (not `eas.json` build profile `env`). The app silently fell back to `http://localhost:8000` for `EXPO_PUBLIC_API_URL` and `USE_MOCK=true` (because `undefined !== 'false'` is `true`), making the device run on mock data for shadowing/home/review while `/ai/simulate/start` failed with `Network Error`. Fixed by `eas env:create` for `EXPO_PUBLIC_API_URL`, `EXPO_PUBLIC_USE_MOCK`, `EXPO_PUBLIC_SENTRY_DSN` in development/preview/production. New OTA `2a73c62c-9131-41b6-ad0e-aa1172df99d2`. User confirmed simulate now opens, home now shows real account data, and shadowing recording no longer self-terminates. Implication: prior `Pass device` rows (D-03, D-05) were effectively mock-path verifications and need re-confirmation on real backend before being trusted as production proof. |
